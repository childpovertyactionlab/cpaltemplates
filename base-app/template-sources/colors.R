...
read_brand_config <- function() {
  brand_path <- "_brand.yml"
  if (file.exists(brand_path)) {
    brand_config <- yaml::read_yaml(brand_path)
    return(brand_config)
  } else {
    stop("Brand configuration file not found in package.")
  }
}

get_primary_color <- function() {
  brand_config <- read_brand_config()
  primary_color <- brand_config$color$primary
